...
class WAIRDDataset(Dataset):
    def __init__(self, data_path: str, scenario: str, split: str):
        ...
        self.__num_envs = 1000
        self.__num_train_envs = 900
        self.__num_val_envs = 100
    ...
    def __getitem__(self, pair_idx: int):
        ...
        return (bs_location.astype(np.float32), ue_location.astype(np.float32),
                toa.astype(np.float32), aod.astype(np.float32), aoa.astype(np.float32), is_los, metadata["img_size"] / 2,
                {"path": os.path.join(self.__scenario_path, environment)})

    def __len__(self):
        return (self.__num_train_envs if self.__split == "train" else self.__num_val_envs) * self.__num_pairs_per_env
    ...
    def __prepare_environments(self) -> list[str]:
        ...
        return environments[:self.__num_train_envs] if self.__split == "train" else environments[self.__num_train_envs:]
