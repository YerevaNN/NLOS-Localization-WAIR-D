...
class UNetERM(pl.LightningModule):
    ...
    def _step(self, batch, *args, **kwargs):
        ...
        ue_location_pred = torch.stack([max_ind % max(pred_image.shape), max_ind // max(pred_image.shape)], dim=1)#.cuda()

        mses_meters = self._mse(ue_location_pred, ue_location).sum(dim=1).sqrt() * image_size / max(pred_image.shape)
